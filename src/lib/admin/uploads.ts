 ...
 *
 * The 32 MB default is Cloud Run's own limit on the size of a request it will
 * accept, so raising this past that on Cloud Run would only move the failure
 * from a clear message to a platform error.
 */
export const MAX_IMAGE_BYTES = 8 * 1024 * 1024;
export const MAX_DOCUMENT_BYTES =
  Number(process.env.MAX_DOCUMENT_MB || 32) * 1024 * 1024;
 ...
 * what a visitor downloads is named the way it was uploaded: the file backend
 * puts it in front of the name (`flatName`) and strips it again when serving,
 * while Cloud Storage puts it in a directory of its own, which leaves the
 * filename itself clean.
 */
